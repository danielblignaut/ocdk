...
import {error, stripExtension, Try, log} from 'json-schema-to-typescript/dist/src/utils'
import {validate} from 'json-schema-to-typescript/dist/src/validator'
import {isDeepStrictEqual} from 'util'
import {link} from 'json-schema-to-typescript/dist/src/linker'
import util from 'util'

export {EnumJSONSchema, JSONSchema, NamedEnumJSONSchema, CustomTypeJSONSchema} from 'json-schema-to-typescript/dist/src/types/JSONSchema'
...
export function compileFromFile(filename: string, options: Partial<Options> = DEFAULT_OPTIONS): Promise<string | null> {
...
export async function compile(schema: JSONSchema4, name: string, options: Partial<Options> = {}): Promise<string | null> {
	...
	if(parsed.dynamodbModel == null || !parsed.dynamodbModel) {
		log('blue', 'parser', time(), '[OCDK: not a dynamo-easy model, lets not do anything]')
		return null
	}
