...
import {log, toSafeString} from 'json-schema-to-typescript/dist/src/utils'
import { DynamoAST, DynamoDbMetadata } from './types'
...
export function generate(ast: AST, options = DEFAULT_OPTIONS): string {
  return (
    [
	  ...
	  declareDynamoEasyClass(ast, options, ast.standaloneName!), //CUSTOM
...
function renderInterfaceToDynamoEasyClass(ast: TInterface  ,options: Options, processedKeys: string[]): [string[], string] {
	const declaration = ast.params
		.filter(_ => !_.isPatternProperty && !_.isUnreachableDefinition)
		.filter(_ => !processedKeys.includes(_.keyName))
		.map(_ => {
			processedKeys.push(_.keyName)

			return _
		})
		.map(
		({isRequired, keyName, ast}) =>
			[isRequired, keyName, ast, generateType(ast, options)] as [boolean, string, DynamoAST, string]
		)
		.map(
		([isRequired, keyName, ast, type]) =>
			(hasComment(ast) && !ast.standaloneName ? generateComment(ast.comment) + '\n' : '') +
			(ast.dynamodbPartitionKey ? `@PartitionKey() \n` : '') +
			(ast.dynamodbSortKey ? `@SortKey() \n` : '') +
			(ast.dynamodbGsiPartitionKey != null && ast.dynamodbGsiPartitionKey.length > 0 ? `@GSIPartitionKey('${ast.dynamodbGsiPartitionKey}') \n` : '') +
			(ast.dynamodbGsiSortKey != null && ast.dynamodbGsiSortKey.length > 0 ? `@GSISortKey('${ast.dynamodbGsiSortKey}') \n` : '') +
			escapeKeyName(keyName) +
			(isRequired ? '' : '?') +
			': ' +
			(hasStandaloneName(ast) ? toSafeString(type) : type)
	...
	return [processedKeys, declaration]
}

function renderDynamoEasyBody(ast: DynamoAST, options: Options, bodyDeclaration = '', processedKeys: string[] = [], index: number = 0): [string[], string] {


	if(ast.type == 'INTERFACE') {
		const [newProcessedKeys, newDeclaration] = renderInterfaceToDynamoEasyClass(ast, options, processedKeys )
		processedKeys = newProcessedKeys
		bodyDeclaration = newDeclaration
	}
	else if(ast.type == 'INTERSECTION') {
		const declarationArr = ast.params.map((param)=> {
			index = index +1
			const [newProcessedKeys, additionalDeclaration] = renderDynamoEasyBody(param, options,bodyDeclaration, processedKeys, index)
			
			return additionalDeclaration
		})

		bodyDeclaration = declarationArr.join(`\n`)
	}

	return [processedKeys, bodyDeclaration]
}

function declareDynamoEasyClass(ast: DynamoAST, options: Options, rootASTName: string, processed = new Set<AST>()): string {
	if (processed.has(ast)) {
	  return ''
	}
	...
	if(!ast.dynamodbModel) {
		return ''
	}

	const [processedKeys, bodyDeclaration] = renderDynamoEasyBody(ast, options)
	


	return (
	  `import { Model, PartitionKey, SortKey, GSISortKey, GSIPartitionKey, DateProperty } from '@shiftcoders/dynamo-easy'\n` +
	  (hasComment(ast) ? generateComment(ast.comment) + '\n' : '') +
	  `
	    @Model()\n
	  	export class ${toSafeString(ast.standaloneName)}_DynamoEasy implements ${toSafeString(ast.standaloneName)} {
			${bodyDeclaration}
		}`
	)
